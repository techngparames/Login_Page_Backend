...
router.post("/add", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "User added", user: newUser });
  } catch (err) {
...
router.post("/daily-face-login", async (req, res) => {
  try {
    ...
    let attendance = await Attendance.findOne({
      ...
      date: { $gte: new Date(`${todayStr}T00:00:00`), $lte: new Date(`${todayStr}T23:59:59`) },
    });
    ...
    res.json({ success: true, message: "Login successful", attendance, user: matchedAdmin });
  } catch (err) {
